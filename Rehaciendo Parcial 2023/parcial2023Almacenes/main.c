...
void ActualizayLista()
{
    ...
    archCostoAlmacen = fopen("CostoAlmacen.dat","rb");

    if ((archExisIniAnt!=NULL)&&(archExistenciaInicial!=NULL)&&(archCostoAlmacen!=NULL))
    {
        ...
        fread(&c,sizeof(CostoAlmacen),1,archCostoAlmacen);
        ...
        // LISTA CON EXISTEINICIAL Y COSTOALMACEN?
        fclose(archExisIniAnt);
        fclose(archExistenciaInicial);
        fclose(archDespacho);
        fclose(archCostoAlmacen);
    }
}
