...
int buscaPosicion(costos vec[],int dimCos,char codActual[], int duracion) // busca posicion y en el genera listado lo llamas con un pos=buscaPosicion
{
    int i=0;
    while (i<dimCos && strcmp(codActual,vec[i].codigo))
    {
        i++;
    }
    while (i<dimCos && !strcmp(codActual,vec[i].codigo) && duracion>=vec[i].duracion)
        i++;
    return i-1;
}
...
void generaListado(costos vec[], int dimCos)
{
    llamadas l;
    FILE *arch;
    int cantLlamdia, acumSegundos,llamMax,pos,llamadaXcelular;
    char numAct[8],fechaAct[11],NumMaxGasto[8];
    float acumGasto,gastoTotal,gastoMax,gastoProm;
    int cantidadCelulares;
    float totalGastoCelulares;
    ...
    if (arch!= NULL)
    {
        ...
        while (!feof(arch))
        {
            ...
            cantidadCelulares++;
            totalGastoCelulares+=gastoTotal;
        ...
        gastoProm = totalGastoCelulares / cantidadCelulares;
        printf("Nro.celular con mayor gasto: %s\n",NumMaxGasto);
        printf("Gasto promedio por celular: %.2f\n", gastoProm);
        fclose(arch);
    }
}
